...
struct key_dev{
    struct device *dev;
    const char *name;
    unsigned int gpio_num; //经过内核映射后的gpio编号
    unsigned int irq_num; //经过内核映射后的中断编号
};

struct tasklet_struct tasklet;

void tasklet_handler(unsigned long data) //中断下半部的服务函数（一般用于执行 耗时但不紧急的操作）
{
...
static int key_probe(struct platform_device *pdev)
{
     ...
     *    为什么要申请资源？
     *    （通过申请 GPIO 资源，内核会检查该引脚是否已经被其他驱动程序占用，
     *      如果没有被占用，内核会将该引脚分配给当前驱动程序，从而保证资源的独占性）
     */
    ...
    pr_info("gpio num = %d\r\n", key_dev->gpio_num); //打印调试信息
    key_dev->irq_num=gpio_to_irq(key_dev->gpio_num); //根据IO编号获取其中断号(注：这里是软件编号只是为了区分，不是硬件芯片手册上的编号)
    pr_info("irq num = %d \n", key_dev->irq_num);
     ...
     *    第五个参数用于 多设备共享中断线）
     */
    ret = devm_request_irq(&pdev->dev, key_dev->irq_num, key_interrupt_handler, IRQF_TRIGGER_FALLING, key_dev->name, key_dev);
    if(ret < 0){
